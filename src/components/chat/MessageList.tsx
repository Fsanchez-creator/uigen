...
function getToolLabel(toolName: string, args: Record<string, unknown>, done: boolean): string {
  const path = typeof args.path === "string" ? args.path : "";
  const filename = path.split("/").pop() || "file";

  if (toolName === "str_replace_editor") {
    switch (args.command) {
      case "create":     return done ? `Created ${filename}`   : `Creating ${filename}`;
      case "str_replace":
      case "insert":     return done ? `Updated ${filename}`   : `Updating ${filename}`;
      case "view":       return done ? `Read ${filename}`      : `Reading ${filename}`;
      case "undo_edit":  return done ? `Reverted ${filename}`  : `Reverting ${filename}`;
      default:           return done ? `Updated ${filename}`   : `Updating ${filename}`;
    }
  }
  ...
  if (toolName === "file_manager") {
    ...
    switch (args.command) {
      case "rename": return done && newFilename ? `Renamed ${filename} → ${newFilename}` : `Renaming ${filename}`;
      case "delete": return done ? `Deleted ${filename}` : `Deleting ${filename}`;
    }
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const label = getToolLabel(tool.toolName, (tool.args ?? {}) as Record<string, unknown>, done);
                            return (
